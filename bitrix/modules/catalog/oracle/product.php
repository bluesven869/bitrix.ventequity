...
class CCatalogProduct extends CAllCatalogProduct
{
	...
	public static function GetQueryBuildArrays($arOrder, $arFilter, $arSelect)
	{
		...
		if (!empty($arJoinGroup))
		{
			...
			{
				$strSql = "SELECT DISTINCT CAT_CG.ID, CAT_CGL.NAME as CATALOG_GROUP_NAME, ".
					"	DECODE(CAT_CGG.ID, NULL, 'N', 'Y') as CATALOG_CAN_ACCESS, ".
					"	DECODE(CAT_CGG1.ID, NULL, 'N', 'Y') as CATALOG_CAN_BUY ".
		...
		$sResFrom .= " left join B_CATALOG_VAT CAT_VAT on (CAT_VAT.ID = DECODE(CAT_PR.VAT_ID, NULL, CAT_IB.VAT_ID, CAT_PR.VAT_ID)) ";
